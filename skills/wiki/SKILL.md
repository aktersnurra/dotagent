...
This reads only each file's frontmatter block. Do not grep whole files for `^tags:` — pages quoting example frontmatter in fenced code blocks would leak those samples into the vocabulary. Note also that ripgrep's `-h` is `--help`, not "no filename"; the no-filename flag is `-I`.
...
Regenerate with the sibling script, resolved by absolute path from this skill's directory:

```sh
/absolute/path/to/skills/wiki/reindex
```

It reads only frontmatter blocks, writes atomically via a temp file, and sets mode `0600`. Do not hand-roll the extraction inline — reading whole files pulls example frontmatter out of fenced code blocks in page bodies.

Never edit the index incrementally, and never treat it as authoritative — pages are the source of truth, the index is a cache. If it is missing or stale during query, regenerate it and continue.
