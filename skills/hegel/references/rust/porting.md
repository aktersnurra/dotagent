...
- Quickcheck infers generators from the function signature via `Arbitrary`; hegel uses
  explicit `tc.draw()` calls.
...
1. **Remove the old dependency** from `Cargo.toml` (if no other tests use it) and add
   hegel.
2. **Replace the test macro/attribute** with `#[hegel::test]`.
3. **Convert strategies/Arbitrary to `tc.draw()` calls.** Start with the broadest
   generators — don't carry over narrow bounds from the old framework unless they're
   justified by the function's contract.
4. **Replace framework-specific assertions** (`prop_assert!`, bool returns) with
   standard `assert!`.
5. **Replace `prop_assume!` / `TestResult::discard()`** with `tc.assume()`.
6. **Simplify dependent generation.** If the old test used `flat_map` chains just to
   make later values depend on earlier ones, rewrite as sequential `tc.draw()` calls.
7. **Remove custom `Shrink` implementations.** Hegel handles shrinking automatically.
8. **Run the tests.** If they fail on inputs the old framework didn't find, investigate
   — that's the point.
