...
test("before_agent_start loads registry overrides, enforces files, and filters the prompt", async () => {
  ...
  await writeFile(join(dir, "skill-visibility.json"), `${JSON.stringify({
    version: 1,
    overrides: { [canonicalDebuggingPath]: "manual", [canonicalWikiPath]: "startup" },
  })}\n`, "utf8");
  ...
  try {
    await withAgentDir(dir, async () => {
      ...
      assert.doesNotMatch(result.systemPrompt, /<name>systematic-debugging<\/name>/);
      assert.match(result.systemPrompt, /<name>wiki<\/name>/);
      assert.match(await readFile(debuggingPath, "utf8"), /disable-model-invocation: true/);
      assert.doesNotMatch(await readFile(wikiPath, "utf8"), /disable-model-invocation/);
    });
  } finally {
...
test("malformed registry falls back to checked-in defaults without overwriting it", async () => {
  const dir = await mkdtemp(join(tmpdir(), "pi-skill-visibility-index-malformed-"));
  const debuggingPath = join(dir, "debugging.md");
  const wikiPath = join(dir, "wiki.md");
  ...
  try {
    await withAgentDir(dir, async () => {
      ...
      assert.match(notifications[0], /Skill visibility policy had 1 error\(s\)/);
