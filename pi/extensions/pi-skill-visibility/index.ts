...
export default function skillVisibilityExtension(pi: ExtensionAPI): void {
  ...
  const registry: Promise<LoadedRegistry> = readVisibilityRegistry(registryPath)
    ...
    .catch((error) => ({
      overrides: {},
      error: bound(`Registry (${registryPath}): ${error instanceof Error ? error.message : String(error)}`),
    }));
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    const modesByPath = new Map(result.resolved.map(({ skill, mode }) => [skill.filePath, mode]));
    ...
    if (!warned && (loadedRegistry.error || result.errors.length > 0 || rewritten.error)) {
      warned = true;
      ...
      const errorCount = result.errors.length + (loadedRegistry.error ? 1 : 0) + (rewritten.error ? 1 : 0);
      ctx.ui.notify(
