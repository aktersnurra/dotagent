...
import { projectSkillVisibility, type EnforcementResult } from "./enforcer.ts";
...
export interface ToggleCommandDependencies {
	...
	resolve(
		skills: Skill[],
		overrides: VisibilityRegistry["overrides"],
	): Promise<ResolvedInventory<Skill>>;
...
export async function runToggleSkillsCommand(
	...
	try {
		...
		ctx.ui.notify(
			`Could not read skill visibility registry at ${path}: ${message(error)}`,
			"error",
		);
		return;
	}
	...
	if (inventory.skills.length === 0) {
		ctx.ui.notify(
			formatErrors("No usable skills found", inventory.errors),
			"warning",
		);
		return;
	}
	...
	const desired = new Map(
		result.drafts.map((draft) => [draft.id, draft.desiredMode]),
	);
	...
	try {
		...
		ctx.ui.notify(
			`Could not save skill visibility registry at ${path}: ${message(error)}`,
			"error",
		);
		return;
	}
...
function formatApplyResult(changes: number, result: EnforcementResult): string {
	const lines = [
		`Applied ${changes} change${changes === 1 ? "" : "s"} to skill visibility.`,
	];
	if (result.errors.length) {
		lines.push(
			`${result.errors.length} file projection${result.errors.length === 1 ? "" : "s"} skipped.`,
		);
		for (const error of result.errors.slice(0, 3)) {
			lines.push(`- ${error.name}: ${bound(error.message)}`);
		}
		if (result.errors.length > 3)
			lines.push(`- … ${result.errors.length - 3} more`);
	}
