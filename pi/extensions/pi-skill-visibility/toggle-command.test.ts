...
test("apply writes minimal registry, projects, notifies, and reloads once", async () => {
  ...
  await runToggleSkillsCommand(state.ctx, dependencies({
    ...
    project: async (affected, expectedFingerprints) => {
      ...
      assert.deepEqual([...expectedFingerprints], [["/wiki/SKILL.md", "popup-open-fingerprint"]]);
...
test("a skill changed while the popup is open is not overwritten", async () => {
  ...
  try {
    ...
    await runToggleSkillsCommand(state.ctx, dependencies({
      ...
      writeRegistry: async (_path, registry) => { savedIntent = registry; },
      project: async (affected, expectedFingerprints) => projectSkillVisibility(
        affected as ResolvedProjection[],
        expectedFingerprints,
      ),
    }));
    assert.deepEqual(savedIntent, { version: 1, overrides: { [file]: "startup" } });
    assert.equal(await readFile(file, "utf8"), concurrent);
    assert.equal(state.reloads(), 1);
    assert.match(state.notices.at(-1)?.[0] ?? "", /changed since the popup opened/);
  } finally {
...
test("source labels cover package, project, temporary, and local skills", async () => {
  ...
  sourceSkills[1]!.sourceInfo = { ...sourceSkills[1]!.sourceInfo, origin: "top-level", scope: "project" };
  sourceSkills[2]!.sourceInfo = { ...sourceSkills[2]!.sourceInfo, origin: "top-level", scope: "temporary" };
  sourceSkills[3]!.sourceInfo = { ...sourceSkills[3]!.sourceInfo, origin: "top-level", scope: "user" };
...
test("reload is the terminal operation after a changed save", async () => {
  ...
  const ctx = {
    hasUI: true,
    getSystemPromptOptions: () => ({ skills: [skill("wiki", "/wiki/SKILL.md")] }),
    ui: {
      notify: () => {
  ...
  await runToggleSkillsCommand(ctx, dependencies({
    showUi: async (_ctx, rows) => ({
      action: "apply",
      drafts: [{ id: rows[0]!.id, desiredMode: "startup" }],
    }),
    writeRegistry: async () => { assert.equal(reloaded, false); },
    project: async () => {
